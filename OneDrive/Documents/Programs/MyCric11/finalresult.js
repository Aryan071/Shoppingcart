...
function displayScoreBoard(index) {
  ...
  document.getElementById(`name${index}`).insertAdjacentHTML("afterbegin", teamName);
  document.getElementById(`fantasyPoints${index}`).insertAdjacentHTML("afterbegin", fantasyPoints);
  document.getElementById(`score${index}`).insertAdjacentHTML("afterbegin", score);
  document.getElementById(`batsman${index}`).appendChild(batsmenData);
  document.getElementById(`bowler${index}`).appendChild(bowlersData);
}

function displayResult() {
  document.getElementById("cricketTime").hidden = true;
  document.getElementById("result").style.display = "block";
  saveTeamsScore();
  let winner = `Winner : ${getWinningTeam()}`;
  document.getElementById(`matchWinner`).insertAdjacentHTML("afterbegin", winner);
  displayScoreBoard(1);
  displayScoreBoard(2);
}
